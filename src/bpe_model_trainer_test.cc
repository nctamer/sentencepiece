...
TEST(BPETrainerTest, ContractNormalizerComparesByValueNotPresence) {
  ...
  // this must reconcile, not fail. The by-value comparison this pins lives in
  // ContinuationTrainer::ReconcileContinuationContract; the test was written
  // on the retired exp/tokenizer-curriculum-v2 branch (193dbce) while the fix
  // itself reached this line separately, so only the regression was missing.
  const std::string input =
